      ...
      <main class="dashboard-main">
        ...
        <section v-if="activeTab === 'dashboard'" class="tab-section">
          ...
          <div class="stats-grid">
            <div v-for="stat in stats" :key="stat.label" class="stat-card" @click="activeTab = stat.tab">
              <div class="stat-icon-wrap" :style="{ background: stat.bg }">
                <Icon :name="stat.icon" class="w-5 h-5" :style="{ color: stat.color }" />
              </div>
              <div>
                <p class="stat-count">{{ stat.count }}</p>
                <p class="stat-label">{{ stat.label }}</p>
              </div>
              <Icon name="mdi:arrow-right" class="w-4 h-4 text-gray-600 ml-auto" />
            </div>
          </div>
          ...
          <div class="dash-welcome">
            ...
            <div>
              <p class="text-white font-semibold">Welcome to your Admin Panel</p>
              <p class="text-gray-400 text-sm">Manage all your portfolio content from the sidebar menu.</p>
            </div>
          </div>
        </section>
        ...
        <section v-if="activeTab === 'projects'" class="tab-section">
          <div class="tab-header">
            <div>
              ...
              <p class="tab-subtitle">{{ projects.length }} project(s) in database</p>
            </div>
        ...
        <section v-if="activeTab === 'experience'" class="tab-section">
          <div class="tab-header">
            <div>
              ...
              <p class="tab-subtitle">{{ experiences.length }} entry(s) in database</p>
            </div>
        ...
        <section v-if="activeTab === 'skills'" class="tab-section">
          <div class="tab-header">
            <div>
              ...
              <p class="tab-subtitle">{{ skills.length }} category(s) in database</p>
            </div>
        ...
        <section v-if="activeTab === 'certificates'" class="tab-section">
          <div class="tab-header">
            <div>
              ...
              <p class="tab-subtitle">{{ certificates.length }} certificate(s) in database</p>
            </div>
      ...
      <div v-if="modal.open" class="modal-backdrop" @click.self="closeModal">
        <div class="modal-box">
          <div class="modal-header">
            <h3 class="modal-title">{{ modal.editing ? 'Edit' : 'Add' }} {{ modalTitle }}</h3>
            <button class="modal-close" @click="closeModal"><Icon name="mdi:close" class="w-4 h-4" /></button>
          </div>

          <div class="modal-body">
            <!-- Project Form -->
            <template v-if="modal.type === 'project'">
              <div class="field-group"><label class="field-label">Title *</label><input v-model="form2.title" class="glass-input" placeholder="e.g. My Awesome Project" /></div>
              <div class="field-group"><label class="field-label">Description *</label><textarea v-model="form2.description" class="glass-input resize-none" rows="3" placeholder="Brief description..." /></div>
              <div class="field-group"><label class="field-label">Tech Stack (comma-separated)</label><input v-model="form2.techStack" class="glass-input" placeholder="e.g. Vue, NestJS, PostgreSQL" /></div>
              <div class="field-group"><label class="field-label">Icon (MDI name)</label><input v-model="form2.icon" class="glass-input" placeholder="e.g. mdi:code-braces" /></div>
              <div class="field-group"><label class="field-label">GitHub URL</label><input v-model="form2.githubUrl" class="glass-input" placeholder="https://github.com/..." /></div>
              <div class="field-group"><label class="field-label">Demo URL</label><input v-model="form2.demoUrl" class="glass-input" placeholder="https://..." /></div>
            </template>

            <!-- Experience Form -->
            <template v-if="modal.type === 'experience'">
              <div class="field-group"><label class="field-label">Role *</label><input v-model="form2.role" class="glass-input" placeholder="e.g. Intern Software Engineer" /></div>
              <div class="field-group"><label class="field-label">Company *</label><input v-model="form2.company" class="glass-input" placeholder="e.g. PT. Example · Internship" /></div>
              <div class="field-group"><label class="field-label">Period *</label><input v-model="form2.period" class="glass-input" placeholder="e.g. Jan 2024 - Jun 2024 · 6 mos" /></div>
              <div class="field-group"><label class="field-label">Location</label><input v-model="form2.location" class="glass-input" placeholder="e.g. Banda Aceh, Indonesia · On-site" /></div>
              <div class="field-group"><label class="field-label">Logo URL</label><input v-model="form2.logo" class="glass-input" placeholder="/images/logo.png" /></div>
              <div class="field-group"><label class="field-label">Tech (comma-separated)</label><input v-model="form2.tech" class="glass-input" placeholder="e.g. Flutter, Dart, REST API" /></div>
            </template>

            <!-- Skill Form -->
            <template v-if="modal.type === 'skill'">
              <div class="field-group"><label class="field-label">Category *</label><input v-model="form2.category" class="glass-input" placeholder="e.g. Frontend" /></div>
              <div class="field-group"><label class="field-label">Icon (MDI name)</label><input v-model="form2.icon" class="glass-input" placeholder="e.g. mdi:palette-outline" /></div>
              <div class="field-group"><label class="field-label">Items (comma-separated)</label><input v-model="form2.items" class="glass-input" placeholder="e.g. React, Vue, Tailwind" /></div>
            </template>

            <!-- Certificate Form -->
            <template v-if="modal.type === 'certificate'">
              <div class="field-group">
                <label class="field-label">Certificate File (Image / PDF)</label>
                <div
                  class="upload-zone"
                ...
                >
                  ...
                  <div v-if="certForm.previewUrl" class="upload-preview">
                    ...
                    <button type="button" class="upload-remove" @click.stop="clearFile"><Icon name="mdi:close" class="w-4 h-4" /></button>
                  </div>
              ...
              <div class="field-group"><label class="field-label">Title *</label><input v-model="form2.title" class="glass-input" placeholder="e.g. AWS Certified Cloud Practitioner" /></div>
              <div class="field-group"><label class="field-label">Description</label><textarea v-model="form2.description" class="glass-input resize-none" rows="2" placeholder="Short description..." /></div>
              <div class="field-group"><label class="field-label">Credential URL</label><input v-model="form2.credentialUrl" class="glass-input" placeholder="https://..." /></div>
            </template>
          </div>
...
// ── Navigation ──
const activeTab = ref<'dashboard' | 'projects' | 'experience' | 'skills' | 'certificates'>('dashboard')
...
const navItems = [
  ...
  { tab: 'certificates', icon: 'mdi:certificate-outline',     label: 'Certificates' },
] as const
...
const stats = computed(() => [
  { label: 'Projects',     count: projects.value.length,     icon: 'mdi:folder-multiple-outline', tab: 'projects',     bg: 'rgba(99,102,241,0.12)',  color: '#818cf8' },
  { label: 'Experiences',  count: experiences.value.length,  icon: 'mdi:briefcase-outline',       tab: 'experience',   bg: 'rgba(16,185,129,0.12)',  color: '#34d399' },
  { label: 'Skill Categories', count: skills.value.length,   icon: 'mdi:code-tags',               tab: 'skills',       bg: 'rgba(245,158,11,0.12)',  color: '#fbbf24' },
  { label: 'Certificates', count: certificates.value.length, icon: 'mdi:certificate-outline',     tab: 'certificates', bg: 'rgba(236,72,153,0.12)',  color: '#f472b6' },
])
...
onMounted(() => { if (isLoggedIn.value) fetchAll() })
...
// ── Modal ──
const modal = reactive({ open: false, type: '' as string, editing: false, editId: null as number | null })
const form2 = reactive<Record<string, any>>({})
const isSaving = ref(false)

const modalTitle = computed(() => {
  const titles: Record<string, string> = { project: 'Project', experience: 'Experience', skill: 'Skill Category', certificate: 'Certificate' }
...
const saveItem = async () => {
  ...
  try {
    if (modal.type === 'certificate') {
      if (!modal.editing) {
        if (!form2.title?.trim() || !certForm.file) { alert('Title and file are required!'); return }
        ...
        await $fetch(`${API_BASE}/certificates`, { method: 'POST', headers, body: fd })
      }
    } else {
      const url = isEdit ? `${API_BASE}/${modal.type}s/${id}` : `${API_BASE}/${modal.type}s`
      const method = isEdit ? 'PATCH' : 'POST'
      await $fetch(url, { method, headers: { ...headers, 'Content-Type': 'application/json' }, body: form2 })
    }
    closeModal()
    await fetchAll()
  } catch (err: any) {
...
const certForm = reactive({ title: '', description: '', credentialUrl: '', previewUrl: '', fileName: '', isPdf: false, file: null as File | null })
...
.blob { position: absolute; border-radius: 50%; filter: blur(100px); opacity: 0.45; top: 50%; left: 50%; }
...
.sidebar-action {
  ...
  font-size: 0.75rem; font-weight: 500; padding: 0.4rem 0.75rem;
  border-radius: 0.5rem; border: 1px solid rgba(255,255,255,0.08);
  color: rgba(180,180,220,0.5); transition: all 0.2s;
}
.sidebar-action:hover { color: rgba(220,220,250,0.9); border-color: rgba(139,92,246,0.3); }
.sidebar-logout:hover { color: #fca5a5; border-color: rgba(239,68,68,0.3); }
...
.stat-icon-wrap {
  width: 2.75rem; height: 2.75rem; border-radius: 0.65rem; flex-shrink: 0;
  display: flex; align-items: center; justify-content: center;
}
.stat-count { font-size: 1.6rem; font-weight: 800; color: #fff; line-height: 1; }
.stat-label { font-size: 0.7rem; color: rgba(180,180,220,0.55); margin-top: 0.15rem; }

.dash-welcome {
  display: flex; align-items: center; gap: 1rem;
  background: rgba(10,8,30,0.4); border: 1px solid rgba(255,255,255,0.08);
...
.list-head--skill { grid-template-columns: 1.5fr 3fr 0.7fr; }

.list-row {
  display: grid; grid-template-columns: 2fr 2fr 1fr 1fr;
  padding: 0.75rem 1.25rem; align-items: center; gap: 0.5rem;
  border-bottom: 1px solid rgba(255,255,255,0.05);
  transition: background 0.2s;
}
.list-row:last-child { border-bottom: none; }
.list-row:hover { background: rgba(255,255,255,0.025); }
.list-row--exp { grid-template-columns: 2fr 2fr 1.5fr 0.7fr; }
.list-row--skill { grid-template-columns: 1.5fr 3fr 0.7fr; }

.empty-state {
  color: rgba(180,180,220,0.35); font-size: 0.85rem;
...
.link-chip {
  ...
  color: rgba(200,200,240,0.6); transition: all 0.2s;
}
.link-chip:hover { background: rgba(139,92,246,0.15); color: #a78bfa; border-color: rgba(139,92,246,0.3); }

/* ── Icon buttons ── */
.icon-btn {
  display: inline-flex; align-items: center; justify-content: center;
  padding: 0.35rem; border-radius: 0.4rem; transition: all 0.2s;
}
.icon-btn--edit { color: rgba(180,180,220,0.4); }
.icon-btn--edit:hover { color: #93c5fd; background: rgba(59,130,246,0.1); }
.icon-btn--del  { color: rgba(180,180,220,0.4); }
.icon-btn--del:hover  { color: #fca5a5; background: rgba(239,68,68,0.1); }
...
.add-btn {
  ...
  border-radius: 0.65rem; color: #fff; white-space: nowrap;
  transition: all 0.25s; box-shadow: 0 4px 15px rgba(124,58,237,0.25);
  flex-shrink: 0;
}
.add-btn:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(124,58,237,0.4); }

/* ── Certificate grid ── */
.cert-grid { display: flex; flex-direction: column; gap: 0.6rem; }
.cert-card {
...
.cert-card:hover { border-color: rgba(139,92,246,0.25); }
.cert-thumb {
  width: 2.75rem; height: 2.75rem; border-radius: 0.5rem; overflow: hidden; flex-shrink: 0;
...
.cancel-btn {
  padding: 0.55rem 1.1rem; font-size: 0.8rem; font-weight: 500;
  border-radius: 0.6rem; border: 1px solid rgba(255,255,255,0.1);
  color: rgba(180,180,220,0.6); transition: all 0.2s;
}
.cancel-btn:hover { border-color: rgba(255,255,255,0.2); color: rgba(220,220,250,0.9); }
...
.upload-preview-img { width: 100%; max-height: 160px; object-fit: contain; }
...
.field-label { font-size: 0.68rem; font-weight: 600; color: rgba(180,180,220,0.6); text-transform: uppercase; letter-spacing: 0.1em; }
.glass-input {
  width: 100%; padding: 0.6rem 0.9rem; background: rgba(255,255,255,0.05);
  border: 1px solid rgba(255,255,255,0.1); border-radius: 0.65rem; color: #fff;
  font-size: 0.85rem; outline: none; transition: all 0.2s;
}
.glass-input::placeholder { color: rgba(200,200,230,0.25); }
.glass-input:focus { border-color: rgba(139,92,246,0.6); box-shadow: 0 0 0 3px rgba(139,92,246,0.12); background: rgba(255,255,255,0.08); }

.logo-text { font-size: 1.5rem; font-weight: 800; background: linear-gradient(135deg, #a78bfa, #60a5fa, #f0abfc); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; display: inline-block; }
...
.submit-btn {
  padding: 0.6rem 1.25rem; background: linear-gradient(135deg, #7c3aed, #4f46e5);
  border-radius: 0.65rem; color: #fff; font-size: 0.85rem; font-weight: 600;
  display: inline-flex; align-items: center; gap: 0.5rem;
  transition: all 0.25s; cursor: pointer;
}
.submit-btn:hover:not(:disabled) { box-shadow: 0 6px 20px rgba(124,58,237,0.4); }
